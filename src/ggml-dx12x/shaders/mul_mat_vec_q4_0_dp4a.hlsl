...
//   block_sum = scale_w * (scale_a * dp4a_sum - 8 * s)
// Per-lane, 8 lanes share each block's dp4a, so per-lane bias = -1.0f * s.
//
    ...
    for (uint block_iter = 0; block_iter < num_blocks; block_iter += BLOCKS_PER_ITER) {
        ...
        if (block_idx < num_blocks) {
            ...
            // --- Row 0 ---
            {
                ...
                // -8 * a_s correction is per-block, but 8 lanes contribute to
                // each block's dp4a, so split it 1/8 per lane.
                acc0 += w_d * (a_d * float(isum) - 1.0f * a_s);
            }
            ...
            // --- Row 1 ---
            {
                ...
                int isum = 0;
                isum = dot4add_i8packed(w_packed, a_packed, isum);
                acc1 += w_d * (a_d * float(isum) - 1.0f * a_s);
            }
        }
    }
