...
// alignment -- on AMD it silently masks the low bits and returns the
// wrong 16 bytes. This helper does an aligned Load4 on the fast path,
// and reconstructs the 16 bytes from 5 aligned word loads on the
// misaligned path. See sibling mul_mat_vec_q6k_dp4a.hlsl for the same
// pattern at u32 granularity.
