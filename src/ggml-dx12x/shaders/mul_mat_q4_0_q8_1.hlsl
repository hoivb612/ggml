// mul_mat_q4_0_q8_1.hlsl - Batch MUL_MAT using dp4a (dot4add_i8packed)
// Weights: Q4_0 (18 bytes/block: d(f16) + qs[16](nibble pairs))
// Input:   Q8_1 (36 bytes/block: ds(2xf16) + qs[32](int8)) in flat scratch buffer
//
// Q4_0 layout: elem l (0..15) -> qs[l] & 0x0F,  elem l+16 -> qs[l] >> 4.
// Signed range [-8, 7].
//
// Trick: feed unsigned q4 values [0,15] to dp4a, correct -8 bias afterwards
// using the Q8_1 's' field (= d_a * sum(a_int8)):
...
//
// One thread = one output cell.  Same structural pattern as
// mul_mat_q8_0_q8_1.hlsl; same dispatch shape; no smem.
...
void main(uint3 group_id : SV_GroupID, uint local_id : SV_GroupIndex) {
    ...
    for (uint block = 0; block < num_blocks; block++) {
        // Q4_0 weight block
        uint w_off = src0_row + block * Q4_0_BSIZE;
        ...
        // 8 dp4a operations per 32-element block:
        //   4 for low nibbles (elems 0..15 vs activations 0..15)
        //   4 for high nibbles (elems 16..31 vs activations 16..31)
        int isum = 0;
        [unroll]
        for (uint j = 0; j < 16; j += 4) {
            uint qs4 = read_u32_fast(src0, w_qs + j);
            uint w_packed_lo = qs4 & 0x0F0F0F0Fu;
            uint i_packed_lo = src1.Load(i_qs + j);
            isum = dot4add_i8packed(w_packed_lo, i_packed_lo, isum);

            uint w_packed_hi = (qs4 >> 4) & 0x0F0F0F0Fu;
            uint i_packed_hi = src1.Load(i_qs + 16 + j);
            isum = dot4add_i8packed(w_packed_hi, i_packed_hi, isum);
        }

        // block_sum = w_d * (a_d * isum - 8 * a_s)
        // a_s already contains d_a * sum(a_int8), so the -8 correction is just -8*a_s.
