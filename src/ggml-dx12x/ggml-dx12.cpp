...
// d3d12_xs.h forward-declares its own DXGI_ADAPTER_DESC1 with the standard
// desktop layout (Description / VendorId / DeviceId / DedicatedVideoMemory /
// SharedSystemMemory / AdapterLuid / Flags), so the rest of this file -- which
// keys vendor-specific perf paths off those fields -- compiles unchanged. We
// just synthesize a value at init_xbox() time.
#endif
...
struct dx12_device {
#if defined(_GAMING_XBOX)
    // No DXGI on the Scarlett partition. Synthesize a desktop-shaped adapter
    // desc at init() so the rest of the file (which keys vendor-specific
    // perf paths off adapter_desc.VendorId / DedicatedVideoMemory) keeps
    // compiling and behaving sanely on Xbox.
#else
    ...
    dx12_shader_blob fused_rms_norm_mul_rope_set_rows_blob{};
    ...
    struct {
        ...
        std::mutex                       mutex; // guards cmd_alloc/cmd_list/fence/upload/readback staging access
    } xfer;
    ...
    // Allocator instrumentation. Counters are always maintained (cost is a
    // few relaxed atomic ops per CreateCommittedResource and is dwarfed by
    // the allocation itself). Per-call logging is opt-in via DX12_TRACE_ALLOC=1.
    // On the final-failure path of dx12_buft_alloc_buffer we always dump a
    // summary, so a single failed run captures: what was tried, in what order,
    // at what cumulative footprint per heap class, and the HRESULT of the
    // first / last failure. This is the empirical data needed to tell
    // "the CUSTOM/L0 path is genuinely full" from "the DEFAULT path can't
    // see any free memory" without speculation.
    struct alloc_stats {
        std::atomic<uint64_t> ok_count{0};
        std::atomic<uint64_t> ok_bytes{0};                // cumulative successful bytes (lifetime)
        std::atomic<uint64_t> ok_max_single{0};           // largest single successful alloc
        std::atomic<uint64_t> fail_count{0};
        std::atomic<uint64_t> fail_first_size{0};         // size of FIRST failure
        std::atomic<uint64_t> fail_first_at_ok_bytes{0};  // ok_bytes when first failure happened
        std::atomic<uint64_t> fail_last_size{0};          // size of MOST RECENT failure
        std::atomic<uint32_t> fail_first_hr{0};
        std::atomic<uint32_t> fail_last_hr{0};
    };
    alloc_stats stats_custom_l0;   // CUSTOM heap, MEMORY_POOL_L0, host-visible (WRITE_BACK)
    alloc_stats stats_default;     // DEFAULT heap (GPU-only fallback path)
...
#if defined(_GAMING_XBOX)
    // Xbox: no DXGI -- single device per partition created via D3D12XboxCreateDevice.
    void init_xbox(size_t idx);
#else
...
static void dx12_ensure_initialized() {
    ...
    // D3D12XboxCreateDevice. The debug layer and experimental-feature
    // surfaces don't exist here -- use PIX-on-Xbox for capture and
    // validation instead.
    {
        ...
        probe_device.Reset(); // dx12_device::init_xbox() will create its own

        g_dx12.devices.push_back(std::make_unique<dx12_device>());
        g_dx12.devices.back()->init_xbox(0);
    }
#else
...
void dx12_device::init_xbox(size_t idx) {
    ...
    // Synthesize an adapter_desc so the vendor/VRAM-keyed paths in the rest of
    // the file behave correctly. Scarlett is a custom AMD RDNA 2 SoC.
    adapter_desc = {};
    adapter_desc.VendorId = 0x1002;  // AMD
    adapter_desc.DeviceId = 0;        // not exposed by GDKX
    size_t title_budget_bytes = (size_t)13ull * 1024 * 1024 * 1024 + (size_t)512 * 1024 * 1024;
    {
        MEMORYSTATUSEX ms = {};
        ms.dwLength = sizeof(ms);
        if (GlobalMemoryStatusEx(&ms) && ms.ullTotalPhys > 0) {
            title_budget_bytes = (size_t)ms.ullTotalPhys;
        }
    }
    adapter_desc.DedicatedVideoMemory  = title_budget_bytes;
    adapter_desc.SharedSystemMemory    = 0; // not applicable
    wcscpy_s(adapter_desc.Description,
    ...
    // Bisection point #1: snapshot before any D3D12 work. Establishes the
    // process baseline (just .dll load + static ctors). Gap between this and
    // "after-device-create" reveals D3D12XboxCreateDevice's hidden cost
    // (queue rings, scratch, PSO/shader pool, tessellation/GS rings, DXR
    // scratch -- everything the driver allocates implicitly).
    dx12_log_memory_status(this, "before-device-create");
    ...
    // Bisection point #2: post-device-create. Defaults (per d3d12_xs.h
    // header comments) are: 4 MB graphics queue ring + 16 MB graphics
    // scratch + 16 MB compute scratch + 64 KB AS args + 64 KB AS payload
    // = ~36 MB explicit. Anything above ~36 MB above the previous snapshot
    // is implicit driver-side allocation (DXR, GS/tess rings, root sig
    // pool, etc.). DisableGeometryShaderAllocations / DisableTessellation /
    // DisableDXR all default to FALSE here.
    dx12_log_memory_status(this, "after-device-create");
    ...
    // Bisection point #3: post-compute-cmdqueue. Should be a small delta;
    // this just creates a queue object.
    dx12_log_memory_status(this, "after-compute-cmdqueue");

    vram_total = adapter_desc.DedicatedVideoMemory;
    if (const char * ovr = getenv("GGML_DX12_VRAM_OVERRIDE_MB")) {
        size_t mb = (size_t)strtoull(ovr, nullptr, 10);
        if (mb > 0) {
    ...
    // Scarlett is architecturally UMA + cache-coherent UMA: a single chip
    // with a single GDDR6 pool shared between Zen 2 cores and RDNA 2 GPU,
    // fully cache-coherent. The GDKX driver, however, presents two memory
    // pools (L0/L1) to mimic the desktop "VRAM + system RAM" split, and
    // CheckFeatureSupport(ARCHITECTURE) often (mis)reports UMA=false on it.
    //
    // When UMA=false, the DEFAULT-heap path only sees the ~10 GB L1 segment
    // and large contiguous allocations (KV cache) fail with E_OUTOFMEMORY
    // even though plenty of title memory is free. Force the correct UMA
    // value so the CUSTOM/L0/WRITE_BACK path is taken -- that path sees the
    // full title budget and is also the zero-copy fast path.
    is_uma = true;
    static const bool no_uma = (getenv("DX12_NO_UMA") != nullptr);
    if (no_uma) is_uma = false;

    // Scarlett caps. CV / WaveMMA not exposed on the GDKX driver today.
    ...
    dx12_log_memory_status(this, "after-root-sig");

    init_shader_blobs();
    // Bisection point #5: post-shader-blobs. All compiled HLSL blobs are
    // now resident. PSOs themselves are created lazily per-dispatch, so
    // this snapshot only includes the blob bytes (a few MiB at most).
    dx12_log_memory_status(this, "after-shader-blobs");

    DX12_LOG_INFO("Device %zu: %s (%s, VRAM: %.1f GB, wave: %u, UMA: %s)\n",
                  idx, name.c_str(), description.c_str(),
                  (double)vram_total / (1024.0 * 1024.0 * 1024.0),
                  wave_size,
                  is_uma ? "yes (forced for Scarlett)" : "no");

    // Final init snapshot. Together with the four above, the deltas show
    // exactly where each MiB of overhead lands during device construction.
    dx12_log_memory_status(this, "init-xbox");
}
#else
void dx12_device::init(ComPtr<IDXGIAdapter1> adapter_, size_t idx) {
    ...
    dx12_log_memory_status(this, "init-pc");
}
#endif // _GAMING_XBOX (init dispatch)
...
//
// ---------------------------------------------------------------------------
// Allocator instrumentation helpers
// ---------------------------------------------------------------------------
//
// One log line per CreateCommittedResource is gated behind DX12_TRACE_ALLOC=1
// (off by default). The atomic counters in dx12_device::alloc_stats are
// updated unconditionally so that the summary dump on a final-failure path
// always has the data, even on a release build with no env vars set.
//
// What gets recorded, per heap class (CUSTOM/L0/WB and DEFAULT):
//   - successful count, cumulative bytes, largest single alloc
//   - failure count, size + HRESULT of the FIRST failure, size + HRESULT of
//     the most recent failure, and ok_bytes at the moment of the first
//     failure (so you can read off "the X-th heap saturated at Y MiB").
//
// What this does NOT record:
//   - Transient staging buffers (UPLOAD/READBACK in xfer_ensure_staging /
//     persistent xfer): they are small, pre-allocated, reused, and not the
//     suspect for a 27 GiB-loaded / 256 MiB-OOM scenario. If they become
//     interesting later this is a one-line addition.
//   - Profiling readback / split-KV / autotune buffers: small one-shot
//     allocations that follow the model load and are not on the failure
//     path we're chasing.

static bool dx12_trace_alloc_enabled() {
    static const bool on = (getenv("DX12_TRACE_ALLOC") != nullptr);
    return on;
}

static void dx12_record_alloc_ok(dx12_device::alloc_stats & s, size_t size) {
    s.ok_count.fetch_add(1, std::memory_order_relaxed);
    s.ok_bytes.fetch_add((uint64_t)size, std::memory_order_relaxed);
    uint64_t prev_max = s.ok_max_single.load(std::memory_order_relaxed);
    while (size > prev_max &&
           !s.ok_max_single.compare_exchange_weak(prev_max, (uint64_t)size,
                                                  std::memory_order_relaxed)) {
        // retry until either size <= prev_max or CAS wins
    }
}

static void dx12_record_alloc_fail(dx12_device::alloc_stats & s, size_t size, HRESULT hr) {
    uint64_t fc = s.fail_count.fetch_add(1, std::memory_order_relaxed);
    if (fc == 0) {
        s.fail_first_size.store((uint64_t)size, std::memory_order_relaxed);
        s.fail_first_hr.store((uint32_t)hr, std::memory_order_relaxed);
        s.fail_first_at_ok_bytes.store(s.ok_bytes.load(std::memory_order_relaxed),
                                       std::memory_order_relaxed);
    }
    s.fail_last_size.store((uint64_t)size, std::memory_order_relaxed);
    s.fail_last_hr.store((uint32_t)hr, std::memory_order_relaxed);
}

// Log the OS-side memory picture so allocation traces can be cross-referenced
// against the actual title-budget pressure rather than just the per-heap
// counters this file maintains.
//
// Xbox (GDKX): the title runs in a fixed memory partition. There is no
// separate "video memory" pool to query -- Scarlett is UMA, so the title
// budget IS the GPU memory budget. The Game OS API surface whitelists
// GlobalMemoryStatusEx (returns title budget total/free in ullTotalPhys/
// ullAvailPhys) and K32GetProcessMemoryInfo (process working set + commit
// charge). Together they answer "how close is this title to the wall?".
// Confirmed via Xbox-GDK-Samples/Samples/System/SystemInfo/SystemInfo.cpp
// (uses GlobalMemoryStatusEx directly) and Samples/Tools/xbdepends/
// GameOSAPIs.h (whitelists both APIs at lines 752 and 835).
//
...
static void dx12_log_memory_status(const dx12_device * dev, const char * tag) {
    ...
    if (GlobalMemoryStatusEx(&ms)) {
        DX12_LOG_WARN("[dx12-os-mem:%s] device=%s GlobalMemoryStatusEx: "
                      "TotalPhys=%.1f MiB AvailPhys=%.1f MiB MemoryLoad=%u%% "
                      "TotalVirtual=%.1f MiB AvailVirtual=%.1f MiB\n",
                      tag, dev->name.c_str(),
                      mib((uint64_t)ms.ullTotalPhys),
                      mib((uint64_t)ms.ullAvailPhys),
                      (unsigned)ms.dwMemoryLoad,
                      mib((uint64_t)ms.ullTotalVirtual),
                      mib((uint64_t)ms.ullAvailVirtual));
    } else {
        DX12_LOG_WARN("[dx12-os-mem:%s] device=%s GlobalMemoryStatusEx FAILED (GetLastError=%lu)\n",
                      tag, dev->name.c_str(), (unsigned long)GetLastError());
    }
    ...
    if (K32GetProcessMemoryInfo(GetCurrentProcess(),
                                ...
                                sizeof(pmc))) {
        DX12_LOG_WARN("[dx12-os-mem:%s] device=%s K32GetProcessMemoryInfo: "
                      "WorkingSet=%.1f MiB PeakWS=%.1f MiB "
                      "PrivateUsage=%.1f MiB PageFaults=%lu\n",
                      tag, dev->name.c_str(),
                      mib((uint64_t)pmc.WorkingSetSize),
                      mib((uint64_t)pmc.PeakWorkingSetSize),
                      mib((uint64_t)pmc.PrivateUsage),
                      (unsigned long)pmc.PageFaultCount);
    } else {
        DX12_LOG_WARN("[dx12-os-mem:%s] device=%s K32GetProcessMemoryInfo FAILED (GetLastError=%lu)\n",
                      tag, dev->name.c_str(), (unsigned long)GetLastError());
    }

#if !defined(_GAMING_XBOX)
    // Desktop: pull the per-segment DXGI numbers as well. dev->adapter is the
    // IDXGIAdapter1 the device was created from; QueryVideoMemoryInfo lives on
    // IDXGIAdapter3 (Win10+, present on every machine that runs DX12).
    if (dev->adapter) {
        ComPtr<IDXGIAdapter3> a3;
        if (SUCCEEDED(const_cast<IDXGIAdapter1 *>(dev->adapter.Get())->QueryInterface(IID_PPV_ARGS(&a3)))) {
            DXGI_QUERY_VIDEO_MEMORY_INFO local = {};
            DXGI_QUERY_VIDEO_MEMORY_INFO nonlocal = {};
            if (SUCCEEDED(a3->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_LOCAL, &local))) {
                DX12_LOG_WARN("[dx12-os-mem:%s] device=%s DXGI LOCAL: "
                              "Budget=%.1f MiB CurrentUsage=%.1f MiB "
                              "AvailableForReservation=%.1f MiB CurrentReservation=%.1f MiB\n",
                              tag, dev->name.c_str(),
                              mib((uint64_t)local.Budget),
                              mib((uint64_t)local.CurrentUsage),
                              mib((uint64_t)local.AvailableForReservation),
                              mib((uint64_t)local.CurrentReservation));
            }
            if (SUCCEEDED(a3->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_NON_LOCAL, &nonlocal))
                && nonlocal.Budget > 0) {
                DX12_LOG_WARN("[dx12-os-mem:%s] device=%s DXGI NON_LOCAL: "
                              "Budget=%.1f MiB CurrentUsage=%.1f MiB\n",
                              tag, dev->name.c_str(),
                              mib((uint64_t)nonlocal.Budget),
                              mib((uint64_t)nonlocal.CurrentUsage));
            }
        }
    }
#endif
...
static void dx12_dump_alloc_stats(const dx12_device * dev, const char * tag) {
    auto mib = [](uint64_t b) { return (double)b / (1024.0 * 1024.0); };

    const auto & sc = dev->stats_custom_l0;
    const auto & sd = dev->stats_default;

    DX12_LOG_WARN("[dx12-alloc-stats:%s] device=%s\n", tag, dev->name.c_str());
    DX12_LOG_WARN("  CUSTOM/L0/WB: ok=%llu cum=%.1f MiB max_single=%.1f MiB | "
                  "fail=%llu first_size=%.1f MiB first_hr=0x%08X first_at_cum=%.1f MiB "
                  "last_size=%.1f MiB last_hr=0x%08X\n",
                  (unsigned long long)sc.ok_count.load(std::memory_order_relaxed),
                  mib(sc.ok_bytes.load(std::memory_order_relaxed)),
                  mib(sc.ok_max_single.load(std::memory_order_relaxed)),
                  (unsigned long long)sc.fail_count.load(std::memory_order_relaxed),
                  mib(sc.fail_first_size.load(std::memory_order_relaxed)),
                  (unsigned)sc.fail_first_hr.load(std::memory_order_relaxed),
                  mib(sc.fail_first_at_ok_bytes.load(std::memory_order_relaxed)),
                  mib(sc.fail_last_size.load(std::memory_order_relaxed)),
                  (unsigned)sc.fail_last_hr.load(std::memory_order_relaxed));
    DX12_LOG_WARN("  DEFAULT     : ok=%llu cum=%.1f MiB max_single=%.1f MiB | "
                  "fail=%llu first_size=%.1f MiB first_hr=0x%08X first_at_cum=%.1f MiB "
                  "last_size=%.1f MiB last_hr=0x%08X\n",
                  (unsigned long long)sd.ok_count.load(std::memory_order_relaxed),
                  mib(sd.ok_bytes.load(std::memory_order_relaxed)),
                  mib(sd.ok_max_single.load(std::memory_order_relaxed)),
                  (unsigned long long)sd.fail_count.load(std::memory_order_relaxed),
                  mib(sd.fail_first_size.load(std::memory_order_relaxed)),
                  (unsigned)sd.fail_first_hr.load(std::memory_order_relaxed),
                  mib(sd.fail_first_at_ok_bytes.load(std::memory_order_relaxed)),
                  mib(sd.fail_last_size.load(std::memory_order_relaxed)),
                  (unsigned)sd.fail_last_hr.load(std::memory_order_relaxed));

    // Always pair the per-heap counters with the OS-side memory picture so
...
static ComPtr<ID3D12Resource> dx12_create_host_visible_buffer(dx12_device * dev,
    ...
    if (FAILED(hr)) {
        dx12_record_alloc_fail(dev->stats_custom_l0, size, hr);
        if (dx12_trace_alloc_enabled()) {
            DX12_LOG_INFO("alloc CUSTOM/L0/WB size=%zu (%.2f MiB) -- CreateCommittedResource FAILED hr=0x%08X cum_ok=%.1f MiB\n",
                          size, (double)size / (1024.0 * 1024.0), (unsigned)hr,
                          (double)dev->stats_custom_l0.ok_bytes.load(std::memory_order_relaxed) / (1024.0 * 1024.0));
        }
        return nullptr;
    }
    ...
    if (FAILED(hr) || !ptr) {
        dx12_record_alloc_fail(dev->stats_custom_l0, size, hr);
        if (dx12_trace_alloc_enabled()) {
            DX12_LOG_INFO("alloc CUSTOM/L0/WB size=%zu (%.2f MiB) -- Map FAILED hr=0x%08X\n",
                          size, (double)size / (1024.0 * 1024.0), (unsigned)hr);
        }
        return nullptr;
    }
    dx12_record_alloc_ok(dev->stats_custom_l0, size);
    if (dx12_trace_alloc_enabled()) {
        DX12_LOG_INFO("alloc CUSTOM/L0/WB size=%zu (%.2f MiB) -- ok cum=%.1f MiB count=%llu\n",
                      size, (double)size / (1024.0 * 1024.0),
                      (double)dev->stats_custom_l0.ok_bytes.load(std::memory_order_relaxed) / (1024.0 * 1024.0),
                      (unsigned long long)dev->stats_custom_l0.ok_count.load(std::memory_order_relaxed));
        // Per-alloc OS-mem snapshot. Lets us cross-reference each per-heap
        // commit with the actual title-budget consumption: if AvailPhys
        // drops by ~size, the buffer is the only thing growing; if it
        // drops by significantly more, something else (driver overhead,
        // OS file cache, etc.) is being charged at the same time.
        dx12_log_memory_status(dev, "after-custom-l0-alloc");
    }
    *mapped_out = ptr;
    return res;
}
...
static ComPtr<ID3D12Resource> dx12_create_buffer(dx12_device * dev, size_t size,
    ...
    HRESULT hr = dev->device->CreateCommittedResource(
        ...
        nullptr, IID_PPV_ARGS(&res));
    if (FAILED(hr)) {
        dx12_record_alloc_fail(dev->stats_default, size, hr);
        DX12_LOG_WARN("CreateCommittedResource(buffer) failed on %s (HRESULT 0x%08X, size=%zu, cum_default_ok=%.1f MiB)\n",
                      dev->name.c_str(), (unsigned)hr, size,
                      (double)dev->stats_default.ok_bytes.load(std::memory_order_relaxed) / (1024.0 * 1024.0));
        return nullptr;
    }
    dx12_record_alloc_ok(dev->stats_default, size);
    if (dx12_trace_alloc_enabled()) {
        DX12_LOG_INFO("alloc DEFAULT      size=%zu (%.2f MiB) -- ok cum=%.1f MiB count=%llu\n",
                      size, (double)size / (1024.0 * 1024.0),
                      (double)dev->stats_default.ok_bytes.load(std::memory_order_relaxed) / (1024.0 * 1024.0),
                      (unsigned long long)dev->stats_default.ok_count.load(std::memory_order_relaxed));
        // Per-alloc OS-mem snapshot (see equivalent comment in the
        // CUSTOM/L0/WB path above).
        dx12_log_memory_status(dev, "after-default-alloc");
    }
    return res;
}
...
static ggml_backend_buffer_t dx12_buft_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (size > 0) {
        ...
        if (dev->is_uma) {
            ...
            //
            // Xbox Scarlett (AMD): the opt-out above does NOT apply -- on the
            // GDKX driver the DEFAULT heap only sees the ~10 GB L1 segment of
            // the title budget, while CUSTOM/L0/WRITE_BACK sees the full
            // ~13.5 GiB. Without this path, large KV caches fail to allocate
            // (E_OUTOFMEMORY) even though plenty of title memory is free.
            constexpr UINT VENDOR_AMD = 0x1002;
#if defined(_GAMING_XBOX_SCARLETT)
            bool skip_custom = false;
#else
            bool skip_custom = (dev->adapter_desc.VendorId == VENDOR_AMD);
#endif
            if (!skip_custom) {
        ...
        if (!ctx->resource) {
            // Both the host-visible (CUSTOM/L0/WB) and GPU-only (DEFAULT) paths
            // failed for this allocation. Dump the full per-heap stats so the
            // log captures the cumulative footprint at the point of failure
            // and the size + HRESULT of the very first failure (which is the
            // most useful data point for diagnosing segment exhaustion vs
            // fragmentation vs budget overrun). This runs unconditionally
            // because allocation failures here are fatal for the caller.
            DX12_LOG_WARN("dx12_buft_alloc_buffer: BOTH paths failed for size=%zu (%.2f MiB) on %s\n",
                          size, (double)size / (1024.0 * 1024.0), dev->name.c_str());
            dx12_dump_alloc_stats(dev, "buft_alloc_buffer-final-failure");
            delete ctx;
            return nullptr;
        }
    }
    ...
    static const ggml_backend_buffer_i iface = {
        ...
        /* .memset_tensor = */ [](ggml_backend_buffer_t buffer, struct ggml_tensor * tensor,
            ...
            std::lock_guard<std::mutex> xfer_lock(ctx->dev->xfer.mutex);
            ctx->dev->init_xfer();
            ctx->dev->xfer_wait();
...
static size_t dx12_buft_get_max_size(ggml_backend_buffer_type_t buft) {
    ...
    constexpr UINT VENDOR_AMD = 0x1002;
#if defined(_GAMING_XBOX_SCARLETT)
    // Scarlett is AMD-UMA but does not exhibit the desktop AMD-iGPU
    // near-4GB driver failure -- keep the normal 4 GiB limit so the title
    // budget isn't fragmented into many small buffers.
    (void)dev; (void)VENDOR_AMD; (void)max_amd_uma_buffer_size;
...
static bool dx12_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_TIMESTEP_EMBEDDING:
            // CPU-computed inside the DX12 dispatch loop (sin/cos embedding;
            // tiny input/output, no GPU shader needed).
            return op->type == GGML_TYPE_F32 &&
                   op->src[0] && op->src[0]->type == GGML_TYPE_F32;
        ...
        case GGML_OP_ARGSORT:
            // Per-row argsort: f32 -> i32 indices via in-shader bitonic sort.
            // Shader pads to next pow2 within BLOCK_SIZE = 256, so the per-row
            // element count must fit. Mixtral has 8 experts; DeepSeek-V2 has 160.
            if (op->type != GGML_TYPE_I32) return false;
            if (!op->src[0] || op->src[0]->type != GGML_TYPE_F32) return false;
            if (op->src[0]->nb[0] != sizeof(float)) return false; // dim-0 must be contiguous
            if (op->src[0]->ne[0] > 256)            return false; // BLOCK_SIZE cap in argsort.hlsl
            return true;
...
static ggml_status dx12_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    // pinpoints exactly which op is in flight if the GPU hangs / TDRs.
    // EXTREMELY slow (kills all pipelining and cmd-list batching) -- diag only.
    static bool sync_dispatch = (getenv("DX12_SYNC_DISPATCH") != nullptr);
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // CPU-host fallback for ops with no GPU shader (sin/cos timestep
        // embedding used by Stable Diffusion UNet). Inputs/outputs are tiny,
        // and the host buffers are CPU-mapped on UMA so we can compute in
        // place. For dGPU (no host mapping), we read src0 via tensor_get and
        // upload the result via tensor_set. Must run on every graph compute
        // (timesteps change per denoising step) — do NOT cache as SKIP.
        if (node->op == GGML_OP_TIMESTEP_EMBEDDING) {
            static bool warned_timestep = false;
            if (!warned_timestep) {
                DX12_LOG_WARN("TIMESTEP_EMBEDDING: no GPU shader, computing on CPU and uploading (per graph)\n");
                warned_timestep = true;
            }
            // Flush any pending GPU work so src0 reads see latest data and
            // subsequent GPU ops see our write.
            bctx->close_and_execute();
            bctx->wait_for_gpu();

            const ggml_tensor * src0 = node->src[0];
            const int dim = ggml_get_op_params_i32(node, 0);
            const int max_period = ggml_get_op_params_i32(node, 1);
            const int half = dim / 2;
            const int64_t N = src0->ne[0];

            std::vector<float> timesteps(N);
            ggml_backend_tensor_get(src0, timesteps.data(), 0, N * sizeof(float));

            std::vector<float> embed((size_t)dim * N, 0.f);
            const float log_max_period = logf((float)max_period);
            for (int64_t n = 0; n < N; n++) {
                float * row = embed.data() + (size_t)n * dim;
                for (int j = 0; j < half; j++) {
                    float freq = expf(-log_max_period * (float)j / (float)half);
                    float arg = timesteps[n] * freq;
                    row[j]        = cosf(arg);
                    row[j + half] = sinf(arg);
                }
                if (dim % 2 != 0) {
            ...
            if (node->nb[1] == row_bytes) {
                ggml_backend_tensor_set(node, embed.data(), 0, embed.size() * sizeof(float));
            } else {
                for (int64_t n = 0; n < N; n++) {
                    ggml_backend_tensor_set(node, embed.data() + (size_t)n * dim,
                                            (size_t)n * node->nb[1], row_bytes);
                }
            }

            bctx->ensure_cmd_list_open();
            bctx->reset_binding_cache();
            // Mark as COMPUTE so replay path does not early-skip via the
            // default-zero (SKIP) kind — it must fall through to this handler
            // on every graph compute.
            if (!no_replay) {
                rcache.decisions[i].kind = DX12_DEC_COMPUTE;
            }
            continue;
        }
        ...
        // Only insert when the current dispatch reads a tensor written by a previous unsynced dispatch.
        {
            ...
            // Debug: DX12_FORCE_BARRIER_PER_DISPATCH=1 inserts a UAV barrier
            // before every single dispatch, regardless of dependency tracking.
            // Equivalent to "BARRIERS_ALL" -- expensive but isolates barrier
            // ordering bugs in one shot. If this fixes a correctness issue,
            // the bug is in the dependency tracker missing a write hazard.
            static const bool force_barrier = (getenv("DX12_FORCE_BARRIER_PER_DISPATCH") != nullptr);
            if (force_barrier) {
                need_barrier = true;
            }
        ...
        // node so we can correlate hangs / TDRs to a specific op. Header is
        // printed at loop top; "ok" footer here. If the loop-top header for
        // node N is the last line in the log, op N is what hung.
        if (sync_dispatch) {
            LARGE_INTEGER sf, ss0, ss1;
            QueryPerformanceFrequency(&sf);
            QueryPerformanceCounter(&ss0);
            bctx->close_and_execute();
            bctx->wait_for_gpu();
            QueryPerformanceCounter(&ss1);
            bctx->ensure_cmd_list_open();
            bctx->reset_binding_cache();
            // Anything we just flushed is now fully visible -- clear the
            // pending-write tracker so the next node doesn't insert a
            // redundant UAV barrier.
            unsynced_writes.clear();
            double sync_ms = (double)(ss1.QuadPart - ss0.QuadPart) * 1000.0 / (double)sf.QuadPart;
            DX12_LOG_INFO("[DX12_SYNC]   ok in %8.3f ms  op=%-15s fl=%u grp=(%u,%u,%u)\n",
                    sync_ms, ggml_op_name(node->op), key.flags,
    ...
    if (do_profile && prof_idx > 0) {
        // The cmd_list may have been closed (and not reopened) by an earlier
        // drain path in this function -- in particular the q8_1_scratch_retired
        // drain a few lines above, or any close_and_execute that was followed
        // by an early `return` of the per-node loop. ResolveQueryData on a
        // closed cmd_list crashes inside Xbox's TinyD3D::Device::Fast_Begin
        // when it tries to allocate a fresh command-buffer segment, so make
        // sure we have an open list before recording the resolve.
        bctx->ensure_cmd_list_open();
...
void dx12_device::run_autotune() {
    ...
    char cache_path[512];
#if defined(_GAMING_XBOX)
    // GDKX has no LOCALAPPDATA. The current working directory is the game's
    // scratch space and is writable, so use that.
    ...
    // Create separate buffers for each binding slot.
    //
    // Earlier this was a single 16 MB buffer aliased at every SRV and UAV
    // slot (src0, src1, dst, src2, src3 all pointing at the same VA). On
    // desktop D3D12 the runtime tolerates this -- the shaders read garbage
    // they write themselves, but the queue scheduler doesn't care because
    // the autotune doesn't validate results. On Scarlett (d3d12_xs) the same
    // workload hard-hangs the kit on the NEXT unrelated command-list submit
    // after autotune completes: the bench dispatches finish cleanly and the
    // result line is printed, then the first real graph_compute dispatch
    // wedges the GPU. De-aliasing the SRV/UAV bindings eliminates the
    // read-modify-write hazard on bench memory and avoids the latent state
    // corruption.
    //
    // Sizing for the largest variant we time (F16 multi-row matvec, K=8192,
    // N=256, 2 rows/group):
    //   src0 (weights):     256 * 8192 * 2  = 4 MB  -> alloc 8 MB for safety
    //   src1 (input/Q8_1):  8192 * 4        = 32 KB -> alloc 1 MB
    //   dst  (output):      256 * 4         = 1 KB  -> alloc 1 MB
    // Total ~10 MB (vs the previous single 16 MB).
    //
    // Slot 4 (src2 SRV) and slot 5 (src3 SRV) are unused by the matvec
    // shaders we benchmark, but the root signature requires *something* to
    // be bound. Aim them at bench_src1 -- it's a valid read-only-from-the-
    // shader-side buffer; the GPU won't touch it via those slots.
    ComPtr<ID3D12Resource> bench_src0;
    ComPtr<ID3D12Resource> bench_src1;
    ComPtr<ID3D12Resource> bench_dst;
    auto create_bench_buf = [&](ComPtr<ID3D12Resource> & out, size_t bytes, const char * name) -> bool {
        D3D12_HEAP_PROPERTIES hp = {}; hp.Type = D3D12_HEAP_TYPE_DEFAULT;
        D3D12_RESOURCE_DESC rd = {};
        rd.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
        rd.Width = bytes;
        ...
        if (FAILED(hr)) {
            DX12_LOG_WARN("Auto-tune: failed to create benchmark %s buffer (HRESULT 0x%08X)\n",
                          name, (unsigned)hr);
            return false;
        }
        return true;
    };
    if (!create_bench_buf(bench_src0, 8 * 1024 * 1024, "src0") ||
        !create_bench_buf(bench_src1, 1 * 1024 * 1024, "src1") ||
        !create_bench_buf(bench_dst,  1 * 1024 * 1024, "dst")) {
        return;
    }
    ...
    auto bench_pipeline = [&](const char * tag, dx12_pipeline_key key, uint32_t K, uint32_t N, uint32_t ts_start) -> uint64_t {
        ...
        // De-aliased bindings: each slot gets its own buffer. Slots 4/5 are
        // unused by these matvec shaders but the root signature still needs
        // a bound resource; src1 is a harmless SRV-compatible dummy.
        D3D12_GPU_VIRTUAL_ADDRESS va_src0 = bench_src0->GetGPUVirtualAddress();
        D3D12_GPU_VIRTUAL_ADDRESS va_src1 = bench_src1->GetGPUVirtualAddress();
        D3D12_GPU_VIRTUAL_ADDRESS va_dst  = bench_dst ->GetGPUVirtualAddress();
        cl->SetComputeRootShaderResourceView(1, va_src0);
        cl->SetComputeRootShaderResourceView(2, va_src1);
        cl->SetComputeRootUnorderedAccessView(3, va_dst);
        cl->SetComputeRootShaderResourceView(4, va_src1);  // dummy (unused)
        cl->SetComputeRootShaderResourceView(5, va_src1);  // dummy (unused)
        ...
        // Order matters on Xbox GDKX: Signal MUST come before SetEventOnCompletion.
        // The desktop D3D12 runtime treats SetEventOnCompletion as a non-blocking
        // registration regardless of order, but the d3d12_xs (Scarlett) runtime
        // appears to block inside SetEventOnCompletion until the fence has
        // already reached the target value. Calling it before Signal therefore
        // hangs forever because the fence never advances.
        //
        // The working pattern used by xfer_wait() and dx12_backend_context::wait_for_fence()
        // is Signal -> SetEventOnCompletion -> WaitForSingleObject, so match that.
        ...
        if (wait_result == WAIT_TIMEOUT) {
            DX12_LOG_WARN("[autotune] %s: GPU benchmark timed out (fence completed=%llu, expected 1)\n",
                          tag, (unsigned long long)fence->GetCompletedValue());
            return UINT64_MAX;
        }
        if (wait_result != WAIT_OBJECT_0) {
            DX12_LOG_WARN("[autotune] %s: WaitForSingleObject unexpected result 0x%08X (GetLastError=%lu)\n",
                          tag, (unsigned)wait_result, (unsigned long)GetLastError());
            return UINT64_MAX;
        }
    ...
    if (dp4a_supported) {
        for (size_t ki = 0; ki < NK; ++ki) {
            ...
            uint64_t t256 = bench_pipeline("Q4_K_dp4a/256t", key256, K, test_N, 0);
            uint64_t t32  = bench_pipeline("Q4_K_dp4a/32t",  key32,  K, test_N, 2);
    ...
    if (dp4a_supported) {
        for (size_t ki = 0; ki < NK; ++ki) {
            ...
            uint64_t t256 = bench_pipeline("Q5_K_dp4a/256t", key256, K, test_N, 0);
            uint64_t t32  = bench_pipeline("Q5_K_dp4a/32t",  key32,  K, test_N, 2);
    ...
    uint64_t f16_mr_256_total = 0, f16_mr_32_total = 0;
    {
        for (size_t ki = 0; ki < NK; ++ki) {
            ...
            uint64_t t256 = bench_pipeline("F16_mr/256t", key256, K, test_N, 0);
            uint64_t t32  = bench_pipeline("F16_mr/32t",  key32,  K, test_N, 2);
